...
@st.cache_data
def generate_vgi_annual_simulation(year, num_ids, pareto_alpha, target_daily_cars, arr_peak_mean, arr_peak_std, dep_peak_mean, dep_peak_std, profile_arrival, run_id):
    """IDベース＆指定来訪台数を考慮したVGIシミュレーションエンジン"""
    np.random.seed(run_id)
    
    # IDプールの生成とパレート分布に基づく来訪確率ウェイト
    ...
    # 平均が1.0になるように正規化（波の形だけを残す）
    mean_factor = np.mean(list(daily_factors.values())) if daily_factors else 1.0
    for k in daily_factors:
        daily_factors[k] = daily_factors[k] / mean_factor
...
with st.sidebar:
    st.header("データソース設定")
    data_mode = st.radio("ソースを選択してください:", ["サンプル: 大学データ", "CSVをアップロード"])
    raw_df = None
    if "CSV" in data_mode:
        uploaded_file = st.file_uploader("1年分の入出庫ログ(CSV)", type=['csv'])
        if uploaded_file: 
...
st.sidebar.divider()
st.sidebar.markdown(f"**推定駐車枠数:** {int(base_capacity)} 台/日")
st.sidebar.markdown(f"**総レコード数:** {len(df):,} 件")
...
# ==========================================
# Tab 3: 日別シミュレーション (以前の形式)
# ==========================================
with tab3:
    st.markdown("### 実データ vs シミュレーション")
    c_s1, c_s2, c_s3 = st.columns([2, 2, 1])
    with c_s1: 
        comp_dt = st.selectbox("比較する日付を選択:", available_dates, key="sb_daily")
    with c_s2: 
        s_cap = st.number_input("目標の駐車台数", 10, 5000, int(base_capacity))
    with c_s3: 
        st.write("") 
...
# ==========================================
with tab4:
    st.markdown("### VGI 年間シミュレーション")
    st.info("「登録ID数」と「1日の平均来訪台数」を設定し、ピークを調整してVGIログを生成します。")
    
    with st.container(border=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            t_yr = st.selectbox("対象年", [2025, 2026, 2027]) # 画像の件を踏まえて2025をデフォルトに追加
        ...
        with c2:
            pareto_alpha = st.slider("ユーザー頻度の偏り (パレート係数)", 1.0, 5.0, 1.5, help="1.0に近いほど一部のヘビーユーザーに偏ります。")
            arr_mean = st.slider("出勤ピーク (平均時刻)", 6.0, 15.0, 9.0, 0.5)
            arr_std = st.slider("出勤ピークの幅", 0.5, 3.0, 1.0, 0.1)
        with c3:
            dep_mean = st.slider("退勤ピーク (平均時刻)", 12.0, 22.0, 17.0, 0.5)
            dep_std = st.slider("退勤ピークの幅", 0.5, 3.0, 1.5, 0.1)
            st.write("")
            if st.button("VGIログを生成する", type="primary", use_container_width=True): 
                st.session_state['annual_run_id'] += 1

    if st.session_state['annual_run_id'] > 0:
        with st.spinner("シミュレーション実行中..."):
            df_events, all_ids = generate_vgi_annual_simulation(
                t_yr, num_ids, pareto_alpha, target_daily,
                arr_mean, arr_std, dep_mean, dep_std, 
                profile_arrival, st.session_state['annual_run_id']
            )
